-- awesome3.5, github.com/f4lls, theimmortal
...
do
    ...
    awesome.connect_signal("debug::error", function (err)
        ...
        naughty.notify({ preset = naughty.config.presets.critical,
                         ...
                         fg = beautiful.fg_yellow, bg = beautiful.fg_black,
                         font = "Tamsyn 8",
                         border_width = 1, border_color = beautiful.border_tooltip,
                         timeout = 0,
                         text = err }) -- opacity = 0.94,
        in_error = false
    end)
end
-- }}}
...
chat             = terminal .. " -title Chat -e weechat"
torrent          = terminal .. " -title Torrent -e torrent"
cli_browser      = terminal .. " -title Elinks -e elinks "
pandora          = terminal .. " -title Pandora -e pandora"
...
function run_once(cmd)
  ...
  --f4lls run_once("xrdb -merge /home/f4lls/.Xdefaults")
...
mytextclockicon:buttons(awful.util.table.join(awful.button({ }, 1, function () awful.util.spawn("xset dpms force off", false) end)))
...
syswidget:buttons(awful.util.table.join(
    awful.button({ }, 1, function () awful.util.spawn(terminale) end),
    awful.button({ }, 3, function () awful.util.spawn(root_terminal) end)))
...
uptimeicon:buttons(awful.util.table.join(awful.button({ }, 1, function () awful.util.spawn(powertop) end)))
...
cpuicon:buttons(awful.util.table.join(awful.button({ }, 1, function () awful.util.spawn(saidar) end)))
...
memicon:buttons(awful.util.table.join(awful.button({ }, 1, function () awful.util.spawn(tasks) end)))
...
fsmenu = awful.menu({items = {
                { "+ All",  function () awful.util.spawn("sh " .. scriptdir .. "/mount-all",   false) end },
                { "- All",  function () awful.util.spawn("sh " .. scriptdir .. "/umount-all",  false) end },
                { "+ Disk", function () awful.util.spawn("sh " .. scriptdir .. "/mount-disk",  false) end },
                { "- Disk", function () awful.util.spawn("sh " .. scriptdir .. "/umount-disk", false) end },
                { "+ Usb",  function () awful.util.spawn("sh " .. scriptdir .. "/mount-usb",   false) end },
                { "- Usb",  function () awful.util.spawn("sh " .. scriptdir .. "/umount-usb",  false) end }}
        })

fsicon:buttons(awful.util.table.join(
    awful.button({ }, 1, function () fsmenu:toggle() end ),
    awful.button({ }, 3, function () awful.util.spawn(cli_fileman) end)))
...
volicon:buttons(awful.util.table.join(awful.button({ }, 1, function () awful.util.spawn(mixer) end)))
...
netdownicon:buttons(awful.util.table.join(awful.button({ }, 1, function () awful.util.spawn("sh " .. scriptdir .. "/restartwifi", false ) end)))
...
pacicon:buttons(awful.util.table.join(awful.button({ }, 1, function () awful.util.spawn(update) end)))
...
--    awful.button({ }, 3, function () mymainmenu:toggle() end), --follow default (f4lls)
...
globalkeys = awful.util.table.join(
    awful.key({ modkey,           }, "Left",   awful.tag.viewprev       ), -- switch between tags.
    awful.key({ modkey,           }, "Right",  awful.tag.viewnext       ), -- idem
    awful.key({ modkey,           }, "Escape", awful.tag.history.restore),
    awful.key({ modkey,           }, "j", -- switch between clients. NOTE: each window running in a tag is a client.
        function ()
            awful.client.focus.byidx( 1)
            if client.focus then client.focus:raise() end
        end),
    awful.key({ modkey,           }, "k", -- focus on one client using modkey j or k
        function ()
            awful.client.focus.byidx(-1)
            if client.focus then client.focus:raise() end
        end),
    ...
    -- Application Switcher (clients in all tags)
     awful.key({ "Mod1" }, "Tab", function ()
     -- If you want to always position the menu on the same place set coordinates
     awful.menu.menu_keys.down = { "Down", "Alt_L" } -- use Alt left or Down to scroll down list of clients.
     local cmenu = awful.menu.clients({width=245}, { keygrabber=true, coords={x=525, y=330} })
     end),
--NOTE: how to use "Tab" to select a client after press Mod1 Tab (Alt Tab)???
    ...
    -- Volume Control
    awful.key({ modkey,         }, "0", function () awful.util.spawn("amixer -q sset Master 1dB+",   false) end),
    awful.key({ modkey,         }, "9", function () awful.util.spawn("amixer -q sset Master 1dB-",   false) end),
    awful.key({ modkey, "Shift" }, "0", function () awful.util.spawn("amixer -q sset Master toggle", false) end),
    ...
    awful.key({ altkey }, ",",     function () awful.util.spawn("mpc random on",            false) end),
    awful.key({ altkey }, ".",     function () awful.util.spawn("mpc random off",           false) end),
    ...
    --f4lls Control Brightness
    awful.key({ "Control" }, "9", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/0", false ) end),
    awful.key({ "Control" }, "1", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/1", false ) end),
    awful.key({ "Control" }, "2", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/2", false ) end),
    awful.key({ "Control" }, "3", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/3", false ) end),
    awful.key({ "Control" }, "4", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/4", false ) end),
    awful.key({ "Control" }, "5", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/5", false ) end),
    awful.key({ "Control" }, "6", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/6", false ) end),
    awful.key({ "Control" }, "7", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/7", false ) end),
    awful.key({ "Control" }, "8", function () awful.util.spawn("sudo sh" .. scriptdir .. "/brightness/8", false ) end),

    --f4lls(deleted all) Applications
...
--    awful.key({ altkey }, "m", function () awful.util.spawn( "dmenu_run -b -nb '#080808' -nf '#aaaaaa' -sf '#778baf' -sb '#080808' -fn '-*-tamsyn-medium-r-normal-*-17-*-*-*-*-*-*-*' -f -h 20 -p 'run application:'", false) end),
--    awful.key({ altkey }, "n", function () awful.util.spawn( "sudo dmenu_run -b -nb '#080808' -nf '#aaaaaa' -sf '#ce5666' -sb '#080808' -fn '*-tamsyn-medium-r-normal-*-17-*-*-*-*-*-*-*' -f -h 20 -p 'run application (root):'", false) end),
--    awful.key({ altkey }, "c", function () awful.util.spawn( "dnetcfg -b -nb '#080808' -nf '#aaaaaa' -sf '#ffaf5f' -sb '#080808' -fn '-*-tamsyn-medium-r-normal-*-17-*-*-*-*-*-*-*' -f -h 20", false) end),
    -- awful.key({ altkey },             "q",                function () awful.util.spawn( "dpm -b -nb '#080808' -nf '#aaaaaa' -sf '#87af5f' -sb '#080808' -fn '-*-tamsyn-medium-r-normal-*-17-*-*-*-*-*-*-*' -f -h 20", false) end),
    
--    awful.key({ altkey },             "w",
--            function ()
--                awful.prompt.run({ prompt = "<span color='" .. beautiful.fg_yellow .. "'>search </span>"},
--                mypromptbox[mouse.screen].widget,
--	            function (command) awful.util.spawn( browser1 .. " 'https://duckduckgo.com/?q=!"..command.."' " )
--	        end)
--        end)
)
...
awful.rules.rules = {
    ...
    { rule = { instance = "plugin-container" },                          properties = { floating = true } }, -- false means watching youtube video bind inside firefox?
